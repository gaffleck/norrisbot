...
NorrisBot.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) &&
        ...
        self.brain.reply(message.text,message.user).then(reply => {
            ...
            var reply = Mustache.render(self.messages[reply.intent][this.language].text,reply.data);//self._formatReply(self.messages[reply.intent],reply.data);
            //console.log('reply data '+JSON.stringify(reply.data));
            self.postTo(channel.name, reply, {as_user: true});
    ...
    if(this._isGroupJoinMessage(message)){
        ...
        var reply = self.messages["CHANNEL_JOIN"][this.language];
        reply.text = Mustache.render(reply.text,null);//self._formatReply(self.messages[reply.intent],reply.data);
            //console.log('reply data '+JSON.stringify(reply.data));
        ...
        reply.username=self.user;
            

        this._api('chat.postMessage', reply);
